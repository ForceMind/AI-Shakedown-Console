...
- Do not present Edge PWA reinstallation as a permanent fix. Recovery order: fully quit the PWA and Edge; compare with Apple's built-in Simplified Pinyin; use a normal browser tab for immediate access. For a durable macOS third-party-IME setup, open the site in Safari and use “File → Add to Dock”; this creates a Safari Web App that does not depend on Edge's App Shim lifecycle. Edge PWA reinstall without clearing browsing data remains a temporary recovery option only.
